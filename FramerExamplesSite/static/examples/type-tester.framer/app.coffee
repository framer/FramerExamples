...
for slider in allSliders
	...
	slider.knob.style.boxShadow = {
		"0 2px 0 rgba(0,0,0,0.1), 0 2px 5px rgba(0,0,0,0.3)"
	}
	...
	valueLayer = new Layer 
		width: 100, height: 120, image: "images/value.png"
		superLayer: slider.knob
		y: slider.knob.y - 120, scale: 0
	...
	# States
	valueLayer.states.add(hide: {scale: 0}, show: {scale: 0.8})
	valueLayer.states.animationOptions = curve: "spring(600,40,0)"
...
# Text input
text = new Layer 
	width: 688, height: 688, x: 32, y: 161
...
# Drag in increments
weightSlider.knob.draggable.updatePosition = (point) ->
...
sizeSlider.on "change:value", -> 
	text.input.style.fontSize = "#{this.value}px"
	this.knob.subLayers[0].html = "#{Math.round(this.value)}"
	
weightSlider.on "change:value", -> 
	text.input.style.fontWeight = "#{round(this.value, 100)}"
	this.knob.subLayers[0].html = round(this.value, 100)
	
spacingSlider.on "change:value", -> 
