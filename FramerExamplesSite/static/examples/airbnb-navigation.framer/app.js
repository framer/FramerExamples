...
/* Setup */
var back, container, front, i, j, len, menu, menuOption, menuOptions, option;
...
menuOptions = (function() {
  ...
  ref = ['Search', 'Discover', 'Your Trips', 'Wish Lists', 'Inbox'];
  results = [];
...
for (i = j = 0, len = menuOptions.length; j < len; i = ++j) {
...
front.on(Events.Click, function() {
  ...
  menu.states.next();
  results = [];
  for (k = 0, len1 = menuOptions.length; k < len1; k++) {
    option = menuOptions[k];
    results.push(option.states.next());
  }
  return results;
});
