...
psd.feed_a.on Events.DragMove, ->	
  ...
  if psd.feed_a.y > 0	
    psd.feed_a.y = psd.feed_a.y / 4  
  
  else
    psd.feed_a.scale = 1 + (psd.feed_a.y * 0.0003)
    psd.feed_a.y = psd.feed_a.y / 2
...
psd.compose.on Events.TouchStart, ->
  if -5 < psd.compose.x < 5	
    psd.compose_icon.animate
      properties: {scale: 0.9}
      curve: click 

psd.compose.on Events.TouchEnd, ->
  if -5 < psd.compose.x < 5
    psd.compose_icon.animate
...
psd.close.on Events.TouchStart, ->
  psd.close_h.opacity = 1
  psd.close_n.opacity = 0	

psd.close.on Events.TouchEnd, ->
  psd.close_h.opacity = 0
