...
recording = new VideoLayer({video: "images/screen.mp4"}) 
recording.width = 300
...
message_button = new Layer width:120, height:32, y:10, backgroundColor:"#fff"
...
follow_button = new Layer width:100, height:32, y:48, backgroundColor:"#fff"
...
psd.follow.visible = false # 加关注页隐藏 Hide follow page
...
psd.m_hi.opacity = 0
psd.message.visible = false # 消息页隐藏 Hide message page
...
Framer.Defaults.Animation = curve: "spring(260,20,0)"
...
# 事件 Events
# 启动icon Start Icon
psd.launch_icon.on Events.TouchStart, ->
  psd.launch_icon.animate
    properties: {scale: 0.9}
    curve: click

psd.launch_icon.on Events.TouchEnd, ->
  psd.desktop.animate
  ...
  psd.splash.on Events.AnimationStop, ->
...
psd.a_icon_like.on Events.Click, ->
  psd.a_like.states.next("small", "big")
  psd.a_liked.states.next("big", "small")
...
psd.b_icon_like.on Events.Click, ->
  psd.b_like.states.next("small", "big")
  psd.b_liked.states.next("big", "small")
...
psd.c_icon_like.on Events.Click, ->
  psd.c_like.states.next("small", "big")
  psd.c_liked.states.next("big", "small")
  
# 设置compose图标 Fade in
psd.compose.on Events.DragMove, ->  
  if psd.compose.x < 0 
  	psd.compose.x = 0
  ...
  psd.feed.blur = 20 - 1/12 * psd.compose.x

psd.compose.on Events.DragEnd, ->
...
psd.compose_icon.on Events.TouchStart, ->
  if -5 < psd.compose.x < 5	
    psd.compose_icon.animate
      properties: {scale: 0.9}
      curve: click 

psd.compose_icon.on Events.TouchEnd, ->
  if -5 < psd.compose.x < 5
    psd.compose_icon.animate
    ...
    recording.player.on "ended", ->
      recording.player.play() 
      
# 设置recording点击返回
# Define the recording interactions
recording.on Events.Click, (event) ->
  recording.animate
...
message_button.on Events.TouchStart, ->
  if psd.compose.x > 10
    message_button.animate
      properties:
        scale: 0.9
...
message_button.on Events.TouchEnd, ->
  if psd.compose.x > 10	
    message_button.animate
      properties:
        scale: 1
...
psd.m_avatar.on Events.TouchStart, ->
  psd.m_avatar.animate
    properties:
      scale: 0.9
    curve: easeout
    
psd.m_avatar.on Events.TouchEnd, ->
  psd.m_avatar.animate
    properties:
      scale: 1
      x: 0
...
# 新消息回复 News reply
psd.m_button.on Events.TouchStart, ->
  psd.m_hi.opacity = 1
  psd.m_no.opacity = 0
  
psd.m_button.on Events.TouchEnd, ->
  psd.m_hi.opacity = 0
  psd.m_no.opacity = 1
...
follow_button.on Events.TouchStart, ->
  if psd.compose.x > 10
    follow_button.animate
      properties:
        scale: 0.9
...
follow_button.on Events.TouchEnd, ->
  if psd.compose.x > 10	
    follow_button.animate
      properties:
        scale: 1
...
psd.button_follow.on Events.TouchStart, ->
  psd.follow_h.opacity = 1
  psd.follow_n.opacity = 0	

psd.button_follow.on Events.TouchEnd, ->
  psd.follow_h.opacity = 0
...
recording.on Events.TouchStart, ->
  psd.close_h.opacity = 1
  psd.close_n.opacity = 0	

recording.on Events.TouchEnd, ->
  psd.close_h.opacity = 0
