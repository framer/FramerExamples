...
doc.photo.on(Events.DragStart, function() {
  if (zoomed === false) {
    doc.photo.animate({
      properties: {
        shadowY: 60,
...
doc.photo.on(Events.DragEnd, function() {
  var down, speed, up;
  doc.bg.animateStop();
  doc.bg.animate({
    properties: {
  ...
  if (zoomed === false) {
    ...
    up.on(Events.AnimationEnd, function() {
      return doc.photo.animate({
        properties: {
          y: 250,
          scale: 1,
          x: 0,
    ...
    down.on(Events.AnimationEnd, function() {
      return doc.photo.animate({
        properties: {
